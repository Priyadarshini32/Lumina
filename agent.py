...
class Agent:
    ...
    def __init__(self, llm_integration, tool_execution_system,
                 terminal_interface, project_root=None):
        """Initializes the Agent with LLM integration, tool system,
        terminal interface, and memory manager."""
        self.llm_integration = llm_integration
        self.tool_execution_system = tool_execution_system
        self.terminal_interface = terminal_interface
        # Use memory manager from tool execution system if available,
        # otherwise create new one
        self.memory_manager = getattr(tool_execution_system, 'memory_manager',
                                      None)
    ...
    def perceive(self, user_input, tool_output=None):
        ...
        if tool_output:
            self.conversation_history.append({"role": "tool_output",
    ...
    def reason(self, perception, is_continuation=False):
        ...
        if is_continuation and perception.get("tool_output"):
            ...
            # analyze output and determine next step
    ...
    def act(self, action):
        ...
        try:
            ...
            if "tool_calls" in response_json and response_json["tool_calls"]:
                ...
                self.terminal_interface.display_message(
                    agent_response_content, title="Agent Response")
                self.conversation_history.append({"role": "model",
                ...
                self.conversation_history.append({"role": "model",
                                                  "content": processed_action})
                return {"status": "success", "message": processed_action,
                        "type": "text_response"}
        ...
        if tool_calls:
            ...
            destructive_actions = [
                "write_file", "delete_file", "clear_file_content",
                "apply_code_change", "edit_file", "edit_notebook",
                "run_terminal_cmd"
            ]
            if function_name in destructive_actions:
                ...
                action_description = (f"The agent wants to execute "
                                      f"'{function_name}' on "
                                      f"'{tool_args.get('filepath', '') or tool_args.get('target_file', '')}'. "
                                      f"Args: {display_args}")
                ...
                if function_name == "write_file":
                    ...
                    preview_content = (f"--- Instructions ---\n"
                                       f"{tool_args.get('instructions', '')}\n"
                                       f"--- Code Edit ---\n"
                                       f"{tool_args.get('code_edit', '')}")
                    language = "diff"
                elif function_name == "edit_notebook":
                    preview_content = (f"--- Old String ---\n"
                                       f"{tool_args.get('old_string', '')}\n"
                                       f"--- New String ---\n"
                                       f"{tool_args.get('new_string', '')}")
                    language = tool_args.get('cell_language', 'text')
                elif function_name == "run_terminal_cmd":
                    preview_content = tool_args.get('command', '')
                    language = "bash"

                if not self.terminal_interface.confirm_action(action_description,
                                                              preview_content,
                                                              language):
                    self.terminal_interface.display_message(
                        "Action cancelled by user.", style="red")
                    self.conversation_history.append({"role": "user_action",
            ...
            if function_name in self.tool_execution_system.available_tools:
                ...
                file_operations = [
                    ...
                    "clear_file_content", "apply_code_change"
                ]
                if function_name in file_operations:
                    filepath = tool_args.get("filepath")
                    if filepath:
                        self.memory_manager.record_file_operation(
                            filepath, function_name, success,
                            error_message=error_message
                        )

                # Cache file content for read operations
                if function_name == "read_file" and \
                   tool_output.get("status") == "success":
                    filepath = tool_args.get("filepath")
                    content = tool_output.get("content", "")
                    if filepath and content:
                        self.memory_manager.cache_file_content(filepath,
                                                               content, "read")
                ...
                self.terminal_interface.display_message(error_message,
                                                        style="red")
                return {"status": "error", "message": error_message,
                        "type": "tool_error"}

        return {"status": "error", "message": "No tool calls detected or text "
                "response from LLM.", "type": "parse_error"}

    def learn(self, observation):
        """Updates memory and context for future decisions."""
        if isinstance(observation, dict) and \
           observation.get("type") == "tool_execution":
    ...
    def run(self, user_input):
        ...
        while iteration_count < self.max_iterations and \
            ...
            if observation and observation.get("type") == "text_response":
                ...
                if ("pytest" in observation.get("content", "") and
                        "failed" in observation.get("content", "")):
                    self.terminal_interface.display_message("Tests failed. "
                                                            "Agent will attempt "
                                                            "to analyze and fix.",
                                                            style="yellow")
                elif ("pytest" in observation.get("content", "") and
                      "passed" in observation.get("content", "")):
                    self.terminal_interface.display_message("Tests passed "
                                                            "successfully!",
                                                            style="green")
