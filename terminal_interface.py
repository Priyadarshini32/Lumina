...
class TerminalInterface:
        ...
        if "```" in message:
            ...
            for i, part in enumerate(parts):
                if i % 2 == 1:
                    ...
                    self.console.print(Panel(syntax, title=title if title else "Code", border_style="yellow"))
                else:
                    if part.strip():
                        # Always wrap agent text/markdown in a Panel
                        self.console.print(Panel(Markdown(part.strip(), style=style), title=title, border_style="green"))
        else:
            # Always wrap agent text/markdown in a Panel
            self.console.print(Panel(Markdown(message, style=style), title=title, border_style="green"))
    ...
    def display_agent_thought(self, thought):
        self.console.print(Panel(Text(f"{thought}", style="italic grey"), title="Agent Thinking", border_style="grey"))
    ...
    def display_tool_call(self, tool_call_json):
        ...
        self.console.print(Panel(tool_text, title="Agent Action", border_style="magenta"))
    ...
    def display_tool_output(self, output):
        ...
        if isinstance(output, dict) and output.get("status") == "success":
            ...
            if content_to_display:
                self.console.print(Panel(Text(f"Tool Execution Successful! [{tool_name}]\nContent:\n{content_to_display}", style="green"), title="Tool Output", border_style="green"))
            else:
                self.console.print(Panel(Text(f"Tool Execution Successful! [{tool_name}]", style="green"), title="Tool Output", border_style="green"))
        elif isinstance(output, dict) and output.get("status") == "error":
            self.console.print(Panel(Text(f"Error in [{tool_name}]: {output['message']}", style="bold red"), title="Tool Output", border_style="red"))
        else:
            self.console.print(Panel(Text(f"Tool Output [{tool_name}]: {output}", style="cyan"), title="Tool Output", border_style="cyan"))

    def confirm_action(self, action_description, preview_content=None, language=None):
        self.console.print(Rule(style="blue"))
        self.console.print(Panel(Text(action_description, style="bold blue"), title="Action Required", border_style="blue"))
        
        if preview_content:
            self.console.print(Panel(Text("Preview of change:", style="italic blue"), title="Preview", border_style="yellow"))
            if language:
                syntax = Syntax(preview_content, language, theme="monokai", line_numbers=True)
                self.console.print(Panel(syntax, title="Code Preview", border_style="yellow"))
            else:
                self.console.print(Panel(Text(preview_content), title="Content Preview", border_style="yellow"))

            # Perform error checking only via file watcher on save; skip in preview
            # (Previously, inline checks ran here for write/edit previews.)
            # This keeps behavior: only saving a .py/.txt/.md file triggers checks.
            #
            # errors = self._check_content_for_errors(filename, preview_content, language_tool_instance=_global_language_tool)
            # if errors:
            #     self._render_error_report(filename, preview_content, errors)
            #     response = self.console.input("[bold yellow]Errors detected. Do you still approve this action? (yes/no)[/bold yellow] ").lower().strip()
            #     if response != 'yes':
            #         return False # User cancelled due to errors

        response = self.console.input("[bold yellow]Do you approve this action? (yes/no)[/bold yellow] ").lower().strip()
        self.console.print(Rule(style="blue"))
        return response == 'yes'

    def _render_error_report(self, file_path, preview_content, errors, allow_apply=False):
        applied = False
        new_content = None
        chosen_suggestion = None
        header = Panel(
            Text(f"Detected issues in manually changed file:\n{file_path}", style="bold red"),
        ...
        # Grouped error list
        if plain_errors:
            error_list = "\n".join([f"{idx}. {e}" for idx, e in enumerate(plain_errors, start=1)])
            self.console.print(Panel(Text(error_list, style="red"), title="Issues", border_style="red"))
        ...
        if suggestion_code_blocks:
            ...
            try:
                original_syntax = Syntax(preview_content, "python", theme="monokai", line_numbers=True)
                suggested_syntax = Syntax(chosen_suggestion, "python", theme="monokai", line_numbers=True)
                left = Panel(original_syntax, title="Original", border_style="red")
                right = Panel(suggested_syntax, title="Suggested Fix", border_style="green")
                self.console.print(Columns([left, right]))
            except Exception:
                syntax = Syntax(chosen_suggestion, "python", theme="monokai", line_numbers=True)
                self.console.print(Panel(syntax, title="Suggested Fix", border_style="green"))
        else:
            # If not Python suggestion, but we have grammar errors, show a concise yellow suggestion list
                ...
                for err in errors:
                    if err.startswith("Grammar Error"):
                        ...
                        if len(top) >= 5:
                            break
                if top:
                    self.console.print(Panel(Text("\n".join(top), style="yellow"), title="Grammar Suggestions", border_style="yellow"))

        # Offer to apply the suggestion when allowed and we have a full path
        if allow_apply and chosen_suggestion and isinstance(file_path, str) and os.path.isabs(file_path):
            with self._input_lock:
                apply_resp = self.console.input("[bold green]Apply the suggested fix to the file? (yes/no)[/bold green] ").lower().strip()
            if apply_resp == 'yes':
                try:
                    ...
                    import tempfile
                    fd, temp_path = tempfile.mkstemp(prefix=base_name + '.', suffix='.tmp', dir=dir_name, text=True)
                    try:
                    ...
                    self.console.print(Panel(Text("Applied suggested fix successfully.", style="bold green"), border_style="green"))
                except Exception as e:
                    self.console.print(Panel(Text(f"Failed to apply fix: {e}", style="bold red"), border_style="red"))

        self.console.print(Rule(style="red"))
        return applied, new_content, chosen_suggestion

    def _check_content_for_errors(self, filename, content, language_tool_instance=None):
        errors = []
        ...
        if file_extension == '.py':
            ...
            try:
                result = subprocess.run(
                    ['flake8', '--stdin-display-name', filename, temp_file_path],
                    capture_output=True, text=True, check=False
                )
                if result.stdout:
                    # Parse flake8 output. Example: 'filename:line:col: E error-code message'
                    for line in result.stdout.splitlines():
                        ...
                        if len(parts) >= 4:
                            ...
                            errors.append(f"Python Linting Error [Code {error_message.split()[0]} | Line {line_num}, Col {col_num}]: {' '.join(error_message.split()[1:])}")
                        else:
                            errors.append(f"Python Linting Error: {line.strip()}")
            except FileNotFoundError:
                errors.append("Python Linting Error: flake8 not found. Please install it (`pip install flake8`).")
            finally:
            ...
            if errors and self.llm_integration:
                ...
                try:
                    llm_suggestion = self.llm_integration.suggest_python_fix(content, lint_blob)
                    if llm_suggestion:
                        errors.append(f"Python Correction Suggestion (from LLM):\n```python\n{llm_suggestion}\n```")
                except Exception:
                    errors.append("Python Correction Suggestion: unavailable (LLM error)")
            ...
            try:
                ...
                for match in matches:
                    ...
                    line = content.count('\n', 0, match.offset) + 1
                    col = match.offset - content.rfind('\n', 0, match.offset) if '\n' in content[:match.offset] else match.offset
                    errors.append(f"Grammar Error [Line {line}, Col {col}]: {message} (Suggestions: {', '.join(match.replacements)})\n")
            except Exception as e:
